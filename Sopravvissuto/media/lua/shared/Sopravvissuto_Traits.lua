-- Sopravvissuto - Tratti creativi per Project Zomboid Build 42.18
--
-- API B42.18 (ricavata dalla vanilla lua clonata):
--   - Registrazione:  TraitFactory.addTrait(id, label, cost, desc, linkedToProf)
--   - Controllo:      player:hasTrait(traitObject)          ← non più HasTrait("string")
--   - Costi B42:      positivo = tratto buono (costa punti)
--                     negativo = tratto brutto (dà punti)   ← invertito rispetto B41
--   - XP boost:       traitObject:addXPBoost(perk, level)
--   - Esclusioni:     traitObject:getMutuallyExclusiveTraits():add(otherTraitObject:getType())
--   - Consumo cibo:   hook su ISEatFoodAction.complete       ← OnItemUse non esiste in B42
--   - Perk corretto:  Perks.Sneak                           ← non Perks.Sneaking
--   - Salute:         getOverallBodyHealth() ritorna 0-100
...
-- ============================================================
-- TRATTI POSITIVI  (costo positivo in B42 = si spendono punti)
-- ============================================================
...
tPredatore:addXPBoost(Perks.Sneak,           1)  -- B42: Perks.Sneak (non Perks.Sneaking)
...
-- ============================================================
-- TRATTI NEGATIVI  (costo negativo in B42 = si guadagnano punti)
-- ============================================================
...
ISEatFoodAction.complete = function(self)
    ...
    if character:hasTrait(tStomDelicato) and item:isRotten and item:isRotten() then
...
-- B42: player:hasTrait(traitObject) — non più HasTrait("string")
-- ============================================================

local function onCreatePlayer(_, player)
    if player:hasTrait(tSangueFreddo) then
        player:getStats():setStress(0)
        player:getStats():setPanic(0)
    end
    if player:hasTrait(tExMilitare) then
        player:getStats():setStress(0)
        player:getStats():setPanic(0)
    end
    if player:hasTrait(tEmofobico) then
        player:getStats():setStress(0.1)
    end
    if player:hasTrait(tGracile) then
        local s = player:getStats()
        s:setFatigue(math.min(1.0, s:getFatigue() + 0.1))
    end
    if player:hasTrait(tZoppo) then
        player:getStats():setPain(0.2)
    end
    if player:hasTrait(tAlcolista) then
        player:getModData()["Soprav_lastAlcohol"] = getGameTime():getWorldAgeHours()
    end
    if player:hasTrait(tCaffeinaDip) then
...
local function onEveryHours()
    ...
    for i = 0, 3 do
        ...
        if player and not player:isDead() then
            ...
            -- TRATTI POSITIVI
            if player:hasTrait(tAdrenalina) and health < 35 then
                stats:setFatigue(math.max(0, stats:getFatigue() - 0.04))
            end
            if player:hasTrait(tNottambulo) and isNight then
            ...
            if player:hasTrait(tEmofobico) and health < 100 then
                stats:setStress(math.min(1.0, stats:getStress() + 0.055))
            end
            if player:hasTrait(tDiurno) and isNight then
            ...
            if player:hasTrait(tIngordo) then
            ...
            if player:hasTrait(tFobicoDelBuio) and isNight and not asleep then
                stats:setStress(math.min(1.0, stats:getStress() + 0.024))
                stats:setPanic(math.min(1.0,  stats:getPanic()  + 0.012))
            end
            if player:hasTrait(tTerroreNotte) and isNight and not asleep then
